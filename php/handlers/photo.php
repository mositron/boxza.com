...
class photo
{
	...
	public function thumb($tid,$fromfile,$todir,$w=120,$h=90,$fix=false,$totype='jpg',$base64=false) // $fix -> both , width , height
	{
		if(!$base64)
		{
			$todir=trim($todir,'/');
		...
		if(isset($image))
		{
			...
			if(!in_array($totype,array('png','gif')) || $fix=='widthheight')
			{
				@imagefill($output, 0, 0, @imagecolorallocate($output, 255, 255, 255));
			}
			elseif(in_array($type,array('png','gif')) && $totype=='jpg')
			{
				@imagefill($output, 0, 0, @imagecolorallocate($output, 255, 255, 255));
			}
			else
			...
			if(!$base64)
			{
				...
				{
					$filename = $tid.'.jpg';
					@imagejpeg($output, $this->folder.$todir.'/'.$filename, 95);
				}
				@imagedestroy($output);
				@imagedestroy($image);
				return $filename;
