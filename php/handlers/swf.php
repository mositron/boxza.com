...
class swf
{
    ...
    */
    function swf($file="")
    {
		...
		if($fp=@fopen($this->file, "rb"))
		{
			...
			if($head == "CWS")
			{
				//$_data = gzuncompress(substr($data,8));
				//$data = substr($data,0,8).gzuncompress(substr($data,8));
				$this->data = substr($data,0,8).gzuncompress(substr($data,8));
				$this->compression = 1;
				$this->isValid = 1;
			} else if ($head == "FWS"){
				$this->data = $data;
    ...
    */
    ...
    {
        if(!$this->is_valid()){
            return ;//;// P-EAR::raiseError("Invalid SWF head TAG found in " . $this->file, PEAR_SWF_ID_ERR);
        }
		if($this->getVersion() >= 8){
			$this->_seek(16);
		} else {
