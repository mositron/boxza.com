...
class html
{
	...
	public function form($fld,$value,$input='input',$prop=array(),$arg=array(),$arg2=array())
	{
		...
		{
			default:
				return array($valshow?$valshow:htmlspecialchars(isset($prop['number_format'])?number_format($value,$prop['number_format']):$value, ENT_NOQUOTES),'<input type="text" class="tbox" size="'.($prop['size']?$prop['size']:'30').'" id="tmp_'.$fld.'" name="tmp_'.$fld.'" value="'.htmlspecialchars($value, ENT_QUOTES).'"'.($prop['required']?' required':'').'>');
				break;
			case 'textarea':
				return array($valshow?$valshow:nl2br(htmlspecialchars($value, ENT_NOQUOTES)),'<textarea class="tbox" cols="'.($prop['cols']?$prop['cols']:'50').'" rows="'.($prop['rows']?$prop['rows']:'5').'" id="tmp_'.$fld.'" name="tmp_'.$fld.'"'.($prop['required']?' required':'').'>'.htmlspecialchars($value, ENT_NOQUOTES).'</textarea>');
			case 'date':
			case 'birth':
				if($input=='birth')
				{
				...
				_::time();
				$html=$this->select2('tmp_'.$fld.'_day',date('j',$value->sec),1,31,1,2,0,$prop);
				...
				$n = date('n',$value->sec);
				for($i=1;$i<=12;$i++)$html.='<option value="'.substr("0".$i,-2).'"'.($n==$i?' selected':'').'>'.time::$month[$i-1].'</option>';
				$html.="</select>\r\n";
				$Y = date('Y',$value->sec);
