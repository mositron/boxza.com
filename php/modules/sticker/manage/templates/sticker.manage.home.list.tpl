...
<?php foreach($this->allorder as $key=>$val):?>
<?php if($key=='p'):?>
<td width="60" class="tab_<?php echo $key?>"><?php if($this->app[$i]['img']):?><img src="http://s3.boxza.com/sticker/cover/<?php echo $this->app[$i]['fd']?>/s.png" style="width:60px"><?php endif?></td>
<?php elseif($key=='s'):?>
...
<?php if($this->app[$i]['pl']):?>
<br><a href="http://sticker.boxza.com/view/<?php echo $this->app[$i]['_id']?>" target="_blank">http://sticker.boxza.com/view/<?php echo $this->app[$i]['_id']?></a>
<?php endif?>
<br><?php echo time::show($this->app[$i]['da'],'datetime')?>
</td>
<?php else:?>
...
<?php endforeach?>
    <td width="60" style="text-align:right">
    	<a href="/manage/<?php echo $this->app[$i]['_id']?>" class="btn btn-mini"><i class="icon-edit"></i></a>
      <a href="javascript:;" onClick="adel(<?php echo $this->app[$i]['_id']?>)" class="btn btn-mini btn-important"><i class="icon-trash"></i></a>
     </td>
   </tr>
<?php endfor?>
