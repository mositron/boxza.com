...
<li><a href="/manage/new"<?php if(_::$path[0]=='new'):?> class="active"<?php endif?>><i class="icon-plus"></i> เพิ่มสติกเกอร์ใหม่</a></li>
</ul>
...
<tr><td class="colum">ชื่อสติกเกอร์ <small>:</small></td><td valign='top'><input type="text" class="span12" name="title" value="<?php echo $this->app['t']?>" maxlength="50" required><br><span class="req">*</span> สูงสุดไม่เกิน 30 ตัวอักษร</td></tr>
<tr><td class="colum">รูปภาพตัวอย่าง <small>:</small></td><td valign='top'>
<div><img src="<?php if($this->app['img']):?>http://s3.boxza.com/sticker/cover/<?php echo $this->app['fd']?>/s.png<?php endif?>" class="prv-img"></div>
...
<div id="imgs" class="row-fluid">
<?php if($this->icon):$i=0;?>
<ul class="thumbnails row-count-4">
<?php foreach($this->icon as $icon):?>
<li class="span3 text-center">
<div><img src="http://s3.boxza.com/sticker/icon/<?php echo $icon['fd']?>/<?php echo $icon['gif']?$icon['gif']:$icon['png']?>" class="prv-img"></div>
<label><input type="checkbox" name="delo[]" value="<?php echo $i?>"> ลบรูปภาพนี้</label>
</li>
<?php endforeach?>
</ul>
<?php endif?>
...
<tr><td class="colum"></td><td valign='top'><input type="submit" class="btn btn-info" value="          บันทึก          ">  หรือกลับไปยัง <a href="/manage" class="btn btn-mini">สติกเกอร์ของฉัน</a></td></tr>
</table>
</form>
