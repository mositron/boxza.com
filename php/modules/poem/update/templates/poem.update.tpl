...
<h2 style="padding:5px; margin:5px; background:#f9f9f9; text-align:center">แก้ไขกลิตเตอร์</h2>
 <form method="post" action="<?php echo URL?>" enctype="multipart/form-data" onSubmit="_.ajax.gourl('<?php echo URL?>','updatepoem',this);return false;" id="sensubmit" class="form-horizontal">
 <fieldset>
...
<?php continue;endif?>
<label style="display:inline-block; width:120px; height:20px; line-height:20px;"><input type="checkbox" name="cate" value="<?php echo $k?>"<?php echo in_array($k,(array)$this->poem['c'])?' checked':''?>> <?php echo $v['t']?></label>
<?php endforeach?>
</div>
...
<div class="controls">
<textarea id="input09" style="height:60px;" class="span7" name="detail" maxlength="1000" minlength="10" required><?php echo $this->poem['t']?></textarea>
...
<div class="controls">
<img src="http://s3.boxza.com/poem/<?php echo $this->poem['fd']?>/l.<?php echo $this->poem['ty']?><?php echo $_SERVER['QUERY_STRING']=='updated'?'?'.rand(1,999):''?>">

<?php if(_::$my['am']):?>
<div><a href="javascript:;" class="btn" onClick="_.box.open('#newpic')">เปลี่ยนรูปภาพ</a></div>
<?php endif?>
</div>
</div>
