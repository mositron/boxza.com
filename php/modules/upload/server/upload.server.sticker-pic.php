...
if($_FILES['file'])
{
	if($_POST['data']['n']&&$_POST['data']['fd']&&$_FILES['file']['tmp_name'])
	{
		...
		_::folder()->mkdir(UPLOAD_FOLDER.'sticker/icon/'.$folder);
		$size=@getimagesize($f);
		$type='png';
		...
		{
			_::photo()->thumb($id,$f,UPLOAD_FOLDER.'sticker/icon/'.$folder,200,200,'inboth','png');
		}
		...
		$status=array('status'=>'OK','data'=>array('ty'=>$type,'n'=>$id,'png'=>$png,'gif'=>$gif,'w'=>$size[0],'h'=>$size[1],'f'=>$_POST['data']['f'],'fd'=>$folder));
