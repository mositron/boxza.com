...
if($_FILES['file'])
{
	if($_POST['data']['type']&&$_POST['data']['folder']&&$_FILES['file']['tmp_name'])
	{
		...
		$cmd='/usr/local/bin/convert '.$_FILES['file']['tmp_name'];
		...
		if($type)
		{
			_::folder()->mkdir(UPLOAD_FOLDER.'image/'.$folder);
			...
			//echo $cmd.$quality.' '.UPLOAD_PATH.'image/'.$folder.'/o.'.$type.' - '.print_r($outl,true);
			//exit;
			$cmd = '/usr/local/bin/convert '.UPLOAD_PATH.'image/'.$folder.'/';
			if($type=='jpg')
			{
			...
			$size=@getimagesize($f);
			$status=array('status'=>'OK','data'=>array('w'=>$size[0],'h'=>$size[1],'s'=>filesize($f)));
		}
		else
		{
