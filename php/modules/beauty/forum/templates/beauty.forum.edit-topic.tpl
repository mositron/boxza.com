...
<form action="<?php echo URL?>" method="post" enctype="multipart/form-data" onSubmit="tinyMCE.triggerSave();" name="post" id="spost">
  <table border="0" cellpadding="3" cellspacing="1" class="forum_post" align="center">
	<tr> 
		<th class="head" height="25">แก้ไขหัวข้อ</th>
	</tr>
	<tr> 
	  <td class="row2"><strong>หัวข้อกระทู้</strong><br><input type="text"  class="tbox" name="title" size="45" maxlength="80" style="width:100%" tabindex="2" value="<?php echo htmlspecialchars(($this->post['title']?$this->post['title']:$this->topic['t']),ENT_QUOTES,'utf-8')?>" /></td>
	</tr>
	<tr> 
      ...
      <div class="forum-upload">
           &nbsp;&bull;&nbsp;<a href="javascript:void(0);" onClick="window.open('http://image.boxza.com/upload?redirect_uri='+encodeURIComponent('http://beauty.boxza.com/forum/addform.html')+'&format=html', '_imagehost', 'resizable=yes,width=600,height=450');return false;">อัพโหลดรูปภาพ</a> - ฝากรูปภาพสูงสุด 10MB, ไม่มีลบ ไม่จำกัดจำนวน
       </div>
...
if(is_array($this->cate[$this->topic['c']]['a']))
{
	...
	{
?>
<tr><td class="row2"><strong>แนบไฟล์รูปภาพ</strong><br>
...
<?php endif?>
<!-- : โค๊ดสำหรับแทรกในเนื้อหาคือ [attach=<?php echo $i+1?>]--></div>
<?php endfor?>
...
?>
<?php if(_::$my['am']):?>
<tr><td class="row2"><strong>ย้ายหมวด</strong> (Admin)<br>
