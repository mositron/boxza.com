...
$template=_::template();
$template->assign('namtoa',new namtoa);
echo $template->fetch2('game.dialog.namtoa');
exit;
...
function doit($_t,$_m)
{
	...
	if(_::$my)
	{
		$namtoa=new namtoa;
		$_m=intval($_m);
		$fmoney=number_format(intval(_::$my['cd']['p']));
		$point=_::point();
		...
		elseif(!in_array($_t,array('1','2','3','4','5','6'))) 
		{
		...
		else
		{
			...
			if($found>0)
			{
				...
				$point->action(_::$my['_id'],$mon,'game','ทายน้ำเต้าถูก ได้รับ '.$mon.' บ๊อก');
				$tmoney=intval(_::$my['cd']['p'])+$mon;
			}else{
				$mon = $_m;
				$res.=" <b style=\"color:#FFFFFF;background:#FF0000\"> ทายผิด </b> - <b>คุณเสีย ".$mon." บ๊อก</b>";
				$point->action(_::$my['_id'],($mon * -1),'game','ทายน้ำเต้าผิด เสีย '.$mon.' บ๊อก');
				$tmoney=intval(_::$my['cd']['p'])-$mon;
			}
			#if($_m>=1000)
			#{
			...
			#}
			$db->insert('game_namtoa',array('u'=>_::$my['_id'],'v'=>$namtoa->input[$_t],'r'=>$res1.','.$res2.','.$res3,'m'=>$_m,'fm'=>$fmoney,'tm'=>number_format($tmoney)));
...
class namtoa
{
	public $input = array(1=>"ไก่", 2=>"น้ำเต้า", 3=>"กุ้ง", 4=>"กบ", 5=>"ปลา", 6=>"ปู");
	public $tamnay=array();
	public $rand_keys;
	...
	public function show()
	{
		...
		{
			$db=_::db();
			$us=$db->find('user',array('_id'=>array('$ne'=>_::$my['_id'])),array('if'=>1),array('sort'=>array('ds'=>-1),'limit'=>20));
			shuffle($us);
			...
			foreach($us as $rs)
			{
				...
				<td width='120' height='120' align='center' valign='middle'><img src='http://s0.boxza.com/static/images/chat/namtoa/".$this->rand_keys[$i].".jpg'></td>
	...
	public function lastplay()
	{
		if(_::$my)
		{
			...
			$tmpdata.="</tr>";
			$i=0;
			if($last=$db->find('game_namtoa',array('u'=>_::$my['_id']),array(),array('sort'=>array('_id'=>-1),'limit'=>10)))
			{
				foreach($last as $rs) 
				{
					$tmpdata.="<tr>";
					$tmpdata.="<td>".time::show($rs['da'],'datetime')."</td>";
					$tmpdata.="<td>".$rs['v']."</td>";
					$tmpdata.="<td>".$rs['m']."</td>";
					$tmpdata.="<td>".$rs['r']."</td>";
					$tmpdata.="<td>".$rs['fm']."</td>";
