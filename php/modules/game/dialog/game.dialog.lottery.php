...
if ($lot=$db->findone('game_lottery',array('fn'=>array('$exists'=>false))))
{
	if($lot['ex']->sec<time())
	{
		$db->update('game_lottery',array('_id'=>$lot['_id']),array('$set'=>array('fn'=>1)));
		$id=$lot['_id'];
		$number=$lot['nb'];
		$time=$lot['ex']->sec;
		$user=_::user();
		$point=_::point();
		###############################  3 ตัว #############################
		if($found=$db->find('game_lottery_answer',array('l'=>$lot['_id'],'n3'=>$lot['n3'],'w'=>array('$exists'=>false))))
		{
			foreach($found as $v)
			{
				$mon = $v['m']*MATCH3_RATE;
				$point->action($v['u'],$mon,'game','ถูกรางวัลเลข 3 ตัว งวดที่ '.$v['_id'].' ได้รับ '.$mon.' บ๊อก');
				$db->update('game_lottery_answer',array('_id'=>$v['_id']),array('$set'=>array('lg'=>'ถูกรางวัลเลข 3 ตัว ได้รับ '.$mon.' บ๊อก','w'=>3)));
			}
		}
		###############################  2 ตัว #############################
		if($found=$db->find('game_lottery_answer',array('l'=>$lot['_id'],'n2'=>$lot['n2'],'w'=>array('$exists'=>false))))
		{
			foreach($found as $v)
			{
				$mon = $v['m']*MATCH2_RATE;
				$point->action($v['u'],$mon,'game','ถูกรางวัลเลข 2 ตัว งวดที่ '.$v['_id'].' ได้รับ '.$mon.' บ๊อก');
				$db->update('game_lottery_answer',array('_id'=>$v['_id']),array('$set'=>array('lg'=>'ถูกรางวัลเลขท้าย 2 ตัว ได้รับ '.$mon.' บ๊อก','w'=>2)));
			}
		}
		###############################  1 ตัว #############################
		if($found=$db->find('game_lottery_answer',array('l'=>$lot['_id'],'n1'=>$lot['n1'],'w'=>array('$exists'=>false))))
		{
			foreach($found as $v)
			{
				$mon = $v['m']*MATCH1_RATE;
				$point->action($v['u'],$mon,'game','ถูกรางวัลเลขท้าย 1 ตัว งวดที่ '.$v['_id'].' ได้รับ '.$mon.' บ๊อก');
				$db->update('game_lottery_answer',array('_id'=>$v['_id']),array('$set'=>array('lg'=>'ถูกรางวัลเลขท้าย 1 ตัว ได้รับ '.$mon.' บ๊อก','w'=>1)));
			}
		}
		lottery_creat();
	}
...
if($lastlot=$db->find('game_lottery',array('fn'=>1),array(),array('sort'=>array('_id'=>-1),'limit'=>1)))
{
	$lastlot=$lastlot[0];
...
echo $template->fetch2('game.dialog.lottery');
exit;
...
function doit($_n,$_m)
{
	...
	if(_::$my)
	{
		...
		else
		{
			if($lot=$db->findone('game_lottery',array('fn'=>array('$exists'=>false))))
			{
				$point->action(_::$my['_id'],($_m*-1),'game','ซื้อล็อตเตอรี่ งวดที่ '.$lot['_id'].' - เลข  '.$_n.' จำนวน '.$_m.' บ๊อก');
				$db->insert('game_lottery_answer',array('u'=>_::$my['_id'],'n'=>_::$my['cname'],'l'=>$lot['_id'],'m'=>$_m,'n'.$len=>$_n));
				...
				if($data=$cache->get('ca2',$key))
				{
					if(is_array($data['text']))
					{
						...
						$al=array(
													...
													't'=>date('H:i',$time),
													'p'=>'',
													'm'=>'[ลอตเตอรี่] - ซื้อเลข '.$_n.' จำนวน '.$_m.' บ๊อก - งวดที่ '.$lot['_id'].' - วันที่ '.time::show($lot['ex'],'date').' - เวลา '.time::show($lot['ex'],'time'),
													'mb'=>1,
													'c'=>1,
													'n'=>_::$my['cname'],
													'l'=>_::$my['link'],
...
function lottery_buy()
{
	...
	&nbsp; &nbsp; <input type="button" value=" ซื้อ " class="button" onClick="_.game.bet.buylot()"></div><br>';
			...
			$i=0;
			if($lots=$db->find('game_lottery_answer',array('l'=>$lot['_id'],'u'=>_::$my['_id']),array(),array('sort'=>array('_id'=>-1))))
			{
				foreach($lots as $rs)
				{
...
function lottery_creat()
{
	...
	if(!$lot=$db->findone('game_lottery',array('fn'=>array('$exists'=>false))))
	{
		...
		{
			$db->insert('game_lottery',array('n3'=>$rand,'n2'=>mb_substr($rand,1,2,'utf-8'),'n1'=>mb_substr($rand,2,1,'utf-8'),'ex'=>new MongoDate(strtotime(date('Y').'-'.date('m').'-'.date('d').' 09:00:00'))));
		}
		elseif(date('G')>8&&date('G')<21)
		{
			$db->insert('game_lottery',array('n3'=>$rand,'n2'=>mb_substr($rand,1,2,'utf-8'),'n1'=>mb_substr($rand,2,1,'utf-8'),'ex'=>new MongoDate(strtotime(date('Y').'-'.date('m').'-'.date('d').' 21:00:00'))));
		}
		else
		{
			$db->insert('game_lottery',array('n3'=>$rand,'n2'=>mb_substr($rand,1,2,'utf-8'),'n1'=>mb_substr($rand,2,1,'utf-8'),'ex'=>new MongoDate(strtotime(date("Y-m-d", (time()+(3600*12))).' 09:00:00'))));
		}
	}
}
...
function lottery_win($number)
{
	...
	if(defined('LOT_ID')&&$win=_::db()->find('game_lottery_answer',array('l'=>LOT_ID,'w'=>$number)))
	{
