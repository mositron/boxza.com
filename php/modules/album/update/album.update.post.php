...
if(!count($error))
{
	...
	$db->update('deal',array('_id'=>$deal['_id']),array(
																																'$set'=>array(
																																									't'=>$_POST['title'],
																																									'l'=>$link,
			...
			switch (strtolower($size['mime']))
			{
				...
				case 'image/x-png':
					if($size[0]>=200 && $size[1]>=200)
					{
						if($i==1)
						{
							$photo->thumb('s',$f,'deal/'.$deal['fd'],75,50,'both','jpg');
							$photo->thumb('m',$f,'deal/'.$deal['fd'],160,120,'both','jpg');
						}
						if($o=$photo->thumb($deal['_id'].'-'.$i,$f,'deal/'.$deal['fd'],600,800,'inboth','jpg'))
						{
							$db->update('deal',array('_id'=>$deal['_id']),array('$set'=>array('o'.$i => $o)));
						}
					}
			}
		}
