...
function getcredit()
{
	...
	if(_::$my)
	{
		...
		if($_GET['cmd']=='private-money')
		{
			...
			{
				if(_::point()->action(_::$my['_id'],50,'event','กิจกรรมรับ 50 บ๊อกต่อวันฟรี'))
				{
		...
		elseif(date('N')==5 && $g>=16 && $g<21)
		{
			...
			if(!$db->findone('point',array('u'=>_::$my['_id'],'ty'=>'event','da'=>array('$gte'=>new MongoDate(strtotime($now.' 00:00:00')),'$lte'=>new MongoDate(strtotime($now.' 23:59:59'))))))
			{
				if(intval(_::$my['st'])>0)
				{
					if(_::point()->action(_::$my['_id'],100,'event','กิจกรรม boxza.com แจกบ๊อกทุกวันศุกร์ฟรี 100 บ๊อก ( สำหรับสมาชิกที่ยืนยันอีเมล์แล้ว )'))
					{
						$ajax->alert('ได้รับบ๊อกเรียบร้อยแล้ว');
						$ajax->script('_.line.go("/line",true,true);');
					}
...
function post($arg)
{
	...
	if(_::$my['_id'])
	{
		...
		if(_::$my['_id']&&trim($arg['msg'])&&(is_array($arg['to'])||$arg['to']!=''))
		{
			...
			$insert = array('u'=>_::$my['_id'],'ty'=>'post','ms'=>strval($html),'rnd'=>rand(0,99));
			$html->clear();
