...
function setban()
{
	if(_::$my['am']>=9)
	{
		...
		$db->remove('football_lnw',array('u'=>_::$profile['_id']));
		$db->remove('football_game',array('u'=>_::$profile['_id']));
			...
			else
			{
				...
				else
				{
					if(_::point()->action(_::$my['_id'],-$co,'gift','ซื้อของขวัญ '.$gf['n'].' มอบให้ <a href="http://boxza.com/'._::$profile['link'].'" target="_blank">'._::$profile['name'].'</a>'))
					{
						...
						$ajax->script('_.line.go("/'._::$profile['link'].'",true,true);');
					}
					else
					{
...
function setblock()
{
	if(_::$my['_id']&&_::$profile['_id']!=_::$my['_id'])
	{
		if(_::$profile['_id']==1)
		{
			_::ajax()->alert('ไม่สามารถปิดกั้นบุคคลนี้ได้');
		}
		else
		{
			$db=_::db();
			$user = _::user();
			$db->remove('friend',array('$or'=>array(array('u'=>_::$my['_id'],'p'=>_::$profile['_id']),array('p'=>_::$my['_id'],'u'=>_::$profile['_id']))));
			$db->remove('notify',array('$or'=>array(array('u'=>_::$my['_id'],'p'=>_::$profile['_id']),array('p'=>_::$my['_id'],'u'=>_::$profile['_id']))));
			$user->update(_::$my['_id'],array('$pull'=>array('ct.fr'=>_::$profile['_id'],'ct.fq'=>_::$profile['_id'])));
			$user->update(_::$profile['_id'],array('$pull'=>array('ct.fr'=>_::$my['_id'],'ct.fq'=>_::$my['_id'])));
			if($u = $user->get(_::$my['_id'],true))
			{
				if(!is_array($u['ct']['bl']))$u['ct']['bl']=array();
				if(!in_array(_::$profile['_id'],$u['ct']['bl']))
				{
					$u['ct']['bl'][]=_::$profile['_id'];
					$user->update(_::$my['_id'],array('$set'=>array('ct.bl'=>array_values(array_filter(array_unique($u['ct']['bl']))))));
				}
			}
			if($ud = $user->get(_::$profile['_id'],true))
			{
				if(!is_array($ud['ct']['bl2']))$ud['ct']['bl2']=array();
				if(!in_array(_::$my['_id'],$ud['ct']['bl2']))
				{
					$ud['ct']['bl2'][]=_::$my['_id'];
					$user->update(_::$profile['_id'],array('$set'=>array('ct.bl2'=>array_values(array_filter(array_unique($ud['ct']['bl2']))))));
				}
			}
			_::ajax()->redirect('/line');
		}
	}
}
