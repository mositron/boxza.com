...
*/
?>
<div>รอดึงข้อมูลถัดไป.. <span id="next"><?php echo $this->place['n']?> (<?php echo $this->place['_id']?>: <?php echo floatval($this->place['loc'][0])?>, <?php echo floatval($this->place['loc'][1])?>)</span></div>
<div id="print_r"></div>
...
function showAddress(results)
{
	...
	_.ajax.gourl('<?php echo URL?>','setlatlng',{'_id':_id,'gg':a})
	$('#print_r').html(c);
}

$(function(){
initialize();
setTimeout(function(){startmap(<?php echo $this->place['_id']?>,<?php echo floatval($this->place['loc'][0])?>,<?php echo floatval($this->place['loc'][1])?>);},5000);
});
</script>