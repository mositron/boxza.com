<div> 
    ...
    <ul class="breadcrumb">
        <li><a href="/" title="ละคร เรื่องย่อละคร"><i class="icon-home"></i> ละคร</a></li>
        <span class="divider">&raquo;</span>
        <li><a href="/<?php echo $this->cate[$this->c]['l']?>" title="<?php echo $this->cate[$this->c]['tt']?>"><?php echo $this->cate[$this->c]['t']?></a></li>
        <span class="divider">&raquo;</span>
        <li> <?php echo $this->drama['t']?> ละคร<?php echo $this->drama['t']?></li>
    </ul>
    <div>
        <h1 class="news-h1"><a href="/<?php echo $this->drama['lk']?>"><?php echo $this->drama['t']?> ละคร<?php echo $this->drama['t']?></a><?php if(_::$my['am']):?><span> ( ดู: <?php echo number_format(intval($this->drama['do']))?> ครั้ง, <a href="http://drama.boxza.com/admin/<?php echo $this->drama['_id']?>">แก้ไข</a> )</span><?php endif?></h1>
        <?php require(HANDLERS.'ads/ads.adsense.body2.php');?>
        <div class="news-detail">
		<?php echo preg_replace('/\<iframe(.*)width="([^"]+)"(.*)height="([^"]+)"(.*)iframe\>/i','<div class="flex-video widescreen"><iframe${1}width="620"${3}height="345"${5}iframe></div>',$this->drama['d']);?>
        ...
        <div style="margin:10px 0px 0px; padding:5px; border-top:1px dashed #ddd;">
            ...
            <div style="margin:0px 0px 0px 55px;"> โดย: <a href="https://plus.google.com/<?php echo $this->user['google']['id']?>?rel=author" rel="author" target="_blank"><?php echo $this->user['google']['name']?></a><br>
                เมื่อ: <?php echo time::show($this->drama['ds'],'datetime')?> </div>
            <?php else:?>
            <div style="float:left; width:50px;"> <a href="http://boxza.com/<?php echo $this->user['link']?>" target="_blank" rel="nofollow"><img src="<?php echo $this->user['img']?>" alt="<?php echo $this->user['name']?>" style="width:45px;"></a> </div>
        ...
        <div class="news-byuser">
            <h4>ละครอื่นๆของ <?php echo $this->user['name']?></h4>
        ...
        <div class="bcd row-fluid">
            <h3 style="background: #f8f8f8;padding: 5px 5px 5px 10px;font-size: 13px;"><i class="l<?php echo $this->c?>"></i> ละครที่ใกล้เคียง</h3>
            <ul class="thumbnails">
                <?php for($i=0;$i<count($this->relate);$i++):?>
                <li class="span2"> <a href="<?php echo link::news($this->relate[$i])?>" target="_blank" class="thumbnail"> <img src="http://s3.boxza.com/drama/<?php echo $this->relate[$i]['fd']?>/s.jpg" alt="<?php echo $this->relate[$i]['t']?>" class="i">
