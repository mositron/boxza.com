...
<li><a href="/list" title="ตรวจหวยย้อนหลัง"><i class="icon-list"></i> ตรวจหวยย้อนหลัง</a></li>
<span class="divider">&raquo;</span>
<li><a href="/<?php echo $this->lotto['_id']?>-<?php echo $this->lotto['l']?>.html" title="ตรวจหวยย้อนหลัง <?php echo time::show($this->lotto['tm'],'date')?>"><i class="icon-star"></i> ตรวจหวย <?php echo time::show($this->lotto['tm'],'date')?></a></li>
</ul>
...
<h2 style="padding:5px; margin:0px 0px 5px 0px; border:1px solid #f0f0f0; border-radius:5px; color:#00ADEF; text-align:center">ตรวจหวย สลากกินแบ่งรัฐบาลงวดที่ <?php echo time::show($this->lotto['tm'],'date')?></h2>
...
<h4> + <a href="<?php echo '/'.$this->last[$i]['_id'].'-'.$this->last[$i]['l'].'.html'?>">ตรวจหวย งวดวันที่ <?php echo time::show($this->last[$i]['tm'],'date')?></a></h4>
<table class="table lotto">
<tbody>
<tr>
<td>
...
<div class="fb-comments" data-href="http://lotto.boxza.com/<?php echo $this->lotto['_id']?>" data-num-posts="30" data-width="710"></div>
