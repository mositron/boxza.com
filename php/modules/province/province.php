...
define('PROV_ID',$pro[SUBDOMAIN]);
				
$template=_::template();

$cache=_::cache();
if(!$data=$cache->get('ca1',SUBDOMAIN.'-global'))
{
	$db=_::db();
	$data=array();
	...
	$province=$db->findone('place',array('_id'=>PROV_ID));
	...
	$cache->set('ca1',SUBDOMAIN.'-global',$data,false,3600);
}
...
$n=$province['n'];
_::$meta['title'] = $n.' - จังหวัด'.$n.' สถานที่ใน'.$n.' โรงแรม ร้านอาหาร  ห้างสรรพสินค้า ร้านค้า โรงเรียน สถานศึกษา ชายทะเล น้ำตก ภูเขา ทั่วประเทศ';
_::$meta['description'] = 'ศูนย์รวมข้อมูลสถานที่ในจังหวัด'.$n.' โรงแรม ร้านอาหาร ห้างสรรพสินค้า ร้านค้า โรงเรียน มหาวิทยาลัย สถานศึกษา ชายทะเล น้ำตก ภูเขา  ตำบล อำเภอ ในจังหวัด'.$n;
_::$meta['keywords'] = $n.', สถานที่, โรงแรม, โรงพยาบาล, โรงเรียน, มหาวิทยาลัย, ห้าง, ร้านอาหาร, ร้านค้า, ตำบล, อำเภอ, จังหวัด, ทั่วประเทศ';
