...
_::$content=$template->fetch('cooked.game');
	...
	if(isset($game[$arg['lv']]))
	{
		...
		if($sc==$max && $fail<$max && USER_ID==$arg['id'] && USER_FB==$arg['fb'])
		{
			...
			if(count($set)>0)
			{
				...
				$db->update('cooked_user',array('_id'=>USER_ID),array('$set'=>$set));
			}
			$fb=array(
								...
								'actions'=>array(array('name'=>'เกมจับคู่+ for Android','link'=>'https://play.google.com/store/apps/details?id=com.doodroid.cooked'))
			);
			if($wall)
			{
