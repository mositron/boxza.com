...
<div class="user"><?php if($this->fb):?><img src="http://graph.facebook.com/<?php echo $this->fb['id']?>/picture?type=square"><p><?php echo $this->fb['name']?></p><div><a href="javascript:;" onClick="relogin()">ออกจากระบบ</a></div><?php endif?></div>
...
{
<?php if(!$this->fb):?>
	$('.user').html('กำลังดึงข้อมูล Facebook');
	FB.api('/me', function(u) {
 		_.ajax.gourl('<?php echo URL?>','setuser',u);
		$('.user').html('<img src="http://graph.facebook.com/'+u.id+'/picture?type=square"><p>'+u.name+'</p><div><a href="javascript:;" onClick="relogin()">ออกจากระบบ</a></div>');
     });
<?php endif?>
}
...
<?php if($this->fb['id']==$this->friend[$i]['fb_id'] || _::$my['_id']==1):?>
...
<?php endif?>
<!--div class="tm"><?php echo str_replace('-','<br>',time::show($this->friend[$i]['da'],'datetime',1))?></div-->
