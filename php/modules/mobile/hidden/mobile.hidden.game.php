...
_::$content=$template->fetch('hidden.game');
	...
	if(isset($game[$arg['lv']]))
	{
		...
		if($sc==$max && $fail<$max && USER_ID==$arg['id'] && USER_FB==$arg['fb'])
		{
			...
			if(count($set)>0)
			{
				...
				$db->update('hidden_user',array('_id'=>USER_ID),array('$set'=>$set));
			}
			$fb=array(
								...
								'description'=>'เกมจับผิด+ by BoxZa.com - เกมทดสอบควมจำ เก็บเลเวล สะสมแต้ม เล่นง่ายๆ บนมือถือ/แท็บเล็ต Android',
								'actions'=>array(array('name'=>'เกมจับผิด+ for Android','link'=>'https://play.google.com/store/apps/details?id=com.doodroid.hidden'))
			);
			if($wall)
			{
