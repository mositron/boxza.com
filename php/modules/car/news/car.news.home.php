...
_::$meta['rss']='http://feed.boxza.com/news-12-'.$c.'/rss';

$_=array('dd'=>array('$exists'=>false),'pl'=>1,'c'=>12,'cs'=>intval($c));
...
if(isset($brand[MODULE_LINK]))
{
	$bth=$brand[MODULE_LINK];
	_::$meta['title']=$cate[$c]['t'].' - รถ'.$bth.' ราคารถใหม่ '.$cate[$c]['t'].' ราคารถยนต์ป้ายแดง '.$cate[$c]['t'].' รถ'.$bth.' ราคารถ'.$bt.'ใหม่ - '.$cate[$c]['t'].' 2013-2014'.($page>1?' (หน้า '.$page.')':'');
}
else
{
