...
if(_::$path[0]=='google')
{
	if($_GET["code"])
	{
		$result=_::http()->get('https://accounts.google.com/o/oauth2/token',array(
																																												...
																																												'redirect_uri'=>  'http://boxza.com/import/google/',
	...
	{
		_::move('https://accounts.google.com/o/oauth2/auth?client_id='._::$config['social']['google']['appid'].'&redirect_uri='.urlencode('http://boxza.com/import/google/').'&scope='.urlencode('https://www.google.com/m8/feeds/').'&response_type=code');
	}
}
...
elseif(_::$path[0]=='live')
{
	...
	if (!empty($_GET['code']))
   {
		...
		$response=_::http()->get('https://oauth.live.com/token',array(
																																												...
																																												'redirect_uri'=>  'http://boxza.com/import/live/?step=1',
																																												'grant_type'=>  'authorization_code',
																																											));
			if ($response !== false)
			{
	 ...
	 elseif(!$_GET['step'])
	 {
		 _::move('https://oauth.live.com/authorize?client_id='._::$config['social']['live']['appid'].'&scope=wl.signin%20wl.basic%20wl.emails&response_type=code&redirect_uri='.rawurlencode('http://boxza.com/import/live/?step=1'));
	 }
}
elseif(_::$path[0]=='twitter')
{
	...
	elseif(!$_GET['step'])
	{
		...
		$tmp =  $connection->getRequestToken('http://boxza.com/import/twitter/?step=1'); // Used applications registered callback URL
