...
function getapp($page=1)
{
	...
	if($count=$db->count('guess',$arg))
	{
		...
		$app=$db->find('guess',$arg,array(),array('skip'=>$skip,'limit'=>$rows,'sort'=>array($order=>($by=='desc'?-1:1))));
	}
...
function newapp($arg)
{
	...
	if(trim($arg['title']))
	{
		if($guess=$db->insert('guess',array('uid'=>_::$my['_id'],'do'=>0,'t'=>mb_substr(trim($arg['title']),0,100,'utf-8'),'ty'=>trim($arg['type']))))
		{		
			$fd = _::folder()->fd($guess);
			$folder = substr($fd,2,2).'/'.substr($fd,4,2);
			$db->update('guess',array('_id'=>$guess),array('$set'=>array('fd'=>substr($fd,2,2).'/'.substr($fd,4,2))));
			_::ajax()->redirect('/manage/'.$guess);
		}
	}
}
...
function delapp($i)
{
	...
	{
		$db->update('guess',array('_id'=>$var['_id']),array('$set'=>array('dd'=>new MongoDate())));
