...
if($f=$_FILES['o']['tmp_name'])
{
	...
	/*elseif($db->findone('glitter',array('md5'=>$md5)))
	{
...
if(!count($error))
{
																						...
																						)))
	{
		...
		$q=_::upload()->send('s3','glitter-post','@'.$f,array('id'=>$id,'folder'=>$folder,'name'=>_::$my['name'],'time'=>time::show(new MongoDate(),'datetime')));
		if($q['status']=='OK')
		{
			$db->update('glitter',array('_id'=>$id),array('$set'=>array('fd'=>$folder,'ty'=>$type,'zp'=>'glitter.boxza.com-'.$id.'.zip')));
		...
		{
			$db->remove('glitter',array('_id'=>$id));
		}
	}
	else
	{
