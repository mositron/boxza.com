...
if($c)
{
	...
	_::$meta['title']=$cate[$c]['t'].' - Glitter กลิตเตอร์'.$cate[$c]['t'].' '._::$meta['title'];
}
_::$meta['description']=_::$meta['title'].', '._::$meta['description'];

_::$meta['rss']='http://feed.boxza.com/glitter'.($c?'-'.$c:'').'/rss';

if($count=$db->count('glitter',$_))
{
	list($pg,$skip)=_::pager()->bootstrap(64,$count,array($url,'page-'),$page);
	$last=$db->find('glitter',$_,array('_id'=>1,'t'=>1,'l'=>1,'fd'=>1,'c'=>1,'cs'=>1,'p'=>1,'ds'=>1,'ty'=>1,'pr'=>1),array('sort'=>array('_id'=>-1),'skip'=>$skip,'limit'=>64));
}
