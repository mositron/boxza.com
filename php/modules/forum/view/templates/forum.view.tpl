...
<style>
.sdc_rpgbar{margin:3px 0px;padding:3px;}
...
<?php if($this->cate[$this->topic['c']]['n']==1 || ($this->cate[$this->topic['c']]['n']==2 && _::$my['am'])):?> <a href="<?PHP echo FORUM_URL?>new-topic/<?php echo $this->topic['c']?>"><i class="forum-new-topic-icon"></i></a> <?php endif?>
<?php if(!$this->topic['lo']):?> <a href="<?PHP echo FORUM_URL?>new-reply/<?php echo $this->topic['_id']?>"><i class="forum-new-reply-icon"></i></a> <?php endif?>
</td>
<td align="right" valign="middle"><?php echo $this->pager?></td>
</tr>
</tbody>
</table>
<h1 class="forum-tt"><i></i> <a href="<?PHP echo FORUM_URL?>topic/<?php echo $this->topic['_id']?>"><?php echo $this->topic['t']?></a></h1>
...
<tr>
    ...
    <?php
	 ...
	 ?>
    <h3><a href="http://boxza.com/<?php echo $this->u['link']?>"><?php echo $this->u['name']?></a></h3>
    <a href="http://boxza.com/<?php echo $this->u['link']?>"><img src="<?php echo $this->u['nimg']?>"></a>
    <?php if(isset($this->forum_mod)&&isset($this->forum_mod[$this->u['_id']])):?>
    <div style="text-align:center"><img src="http://s0.boxza.com/static/images/forum/class/<?php echo $this->forum_mod[$this->u['_id']]?>.png" alt="<?php echo $this->forum_mod[$this->u['_id']]?>"></div>
    <?php else:?>
    ...
    <tr><td><a href="<?PHP echo FORUM_URL?>topic/<?php echo $this->topic['_id']?>" rel="nofollow" class="forum-nav-link"><i class="forum-calendar-icon"></i> <?php echo str_replace('-','<i class="forum-time-icon"></i>',time::show($this->topic['da'],'datetime',true))?></a><?php if(_::$my['am']):?> - IP: <?php echo $this->topic['ip']?> <?php endif?>
     - อ่าน: <?php echo number_format(intval($this->topic['do']))?> - ตอบ: <?php echo number_format(intval($this->topic['cm']['c']))?>
    </td>
    <td align="right">
    <?php if(in_array(_::$type,array('forum','entertain'))&&_::$my['am']):?>
    <!--span style="vertical-align:text-bottom">
    <?php if($this->topic['ads']):?>
    [<a href="javascript:;" onClick="_.ajax.gourl('<?php echo URL?>','showads',0)">ซ๋อน Adsense</a>
    <?php else:?>
    [<a href="javascript:;" onClick="_.box.confirm({'title':'ยืนยันการแสดงโฆษณาจาก Google','detail':'กระทู้นี้ ไม่มีภาพหรือเนื้อหา รุนแรง อนาจาร เสียว หรือโป๊เปลีอยใช้หรือไม่','click':function(){_.ajax.gourl('<?php echo URL?>','showads',1)}})">แสดง Adsense</a>]
    <?php endif?>
    </span-->
    <?php endif?>
    <?php if(!$this->topic['lo']):?>
	...
	<?php if(!in_array($this->topic['c'],array(482,542,543,544,545))):?>
   ...
   <?php endif?>
	<?php if(!in_array($this->topic['c'],array(482,542,543,544,545))):?>
   <?php require(HANDLERS.'ads/ads.yengo.body2-2.php');?>
	<?php endif?> 
...
<?php if($this->relate):?>
<?php if(in_array($this->topic['c'],array(38,412,413,414,415,416,417,418,419,420,481))):?>
...
<li>
<div class="l"><a href="<?PHP echo FORUM_URL?>topic/<?php echo $this->relate[$i]['_id']?>"><?php if($this->relate[$i]['s']):?><img src="http://s3.boxza.com/forum/<?php echo $this->relate[$i]['fd']?>/t.jpg" alt="<?php echo $this->relate[$i]['t']?>"><?php endif?></a></div>
<div class="r">
<p><a href="<?PHP echo FORUM_URL?>topic/<?php echo $this->relate[$i]['_id']?>"><?php echo $this->relate[$i]['t']?></a></p>
</div>
<p class="clear"></p>
</li>
<?php endfor?>
...
<?php endif?>  
 <div class="fo-view-love">
   ...
   <p class="clear"></p>
    ...
    <hr class="hr">
    <div class="sig"><?php echo str_ireplace(array('<iframe','/iframe>','<script','/script>'),array('<!--iframe','/iframe-->','<!--script','/script-->'),$this->u['sg'])?></div>
    </td>
</tr>
<?php endif?>
...
<tr>
    ...
    <?php if(isset($this->forum_mod)&&isset($this->forum_mod[$this->u['_id']])):?>
    <div style="text-align:center"><img src="http://s0.boxza.com/static/images/forum/class/<?php echo $this->forum_mod[$u['_id']]?>.png" alt="<?php echo $this->forum_mod[$u['_id']]?>"></div>
    <?php else:?>
    ...
    <div class="cedit">แก้ไขโดย <a href="http://boxza.com/<?php echo $eu['link']?>"><?php echo $eu['name']?></a> เมื่อ <?php echo time::show($this->topic['cm']['d'][$i]['e'][$e]['t'],'datetime',true)?></div>
	 <?php endif?>
    <hr class="hr">
	<div class="sig"><?php echo str_ireplace(array('<iframe','/iframe>','<script','/script>'),array('<!--iframe','/iframe-->','<!--script','/script-->'),$u['sg'])?></div>
   <?php else:?>
...
<h4>ตอบกระทู้ด่วน</h4>
<?php if(_::$my):?>
<div style="padding:5px; text-align:center; border:1px solid #f00; color:#f00; margin:5px 0px;">ห้ามโฆษณาเว็บ หรือโปรโมทเว็บ<strong>โดยเด็ดขาด</strong>!. หากฝ่าฝืน ลบกระทู้+ลบโพสต์+แบนไอดีสมาชิกทันที!.</div>
<div>
