...
function sendreport($arg)
{
	...
	if($f=$db->findone('msn',array('_id'=>intval($arg['friend']),'dd'=>array('$exists'=>false))))
	{
		 ...
		 elseif($arg['reason'])
		 {
			$mail=_::mail();
			$mail->to=$f['em'];
			$mail->subject = 'ลิ้งค์สำหรับลบข้อความ - BoxZa Friend หาเพื่อน หาแฟน หากิ๊ก หาคู่';
			$template->assign('f',$f);
			$template->assign('code',md5($f['_id'].'+d+'.$f['em']));
			$mail->message = $template->fetch('home.report');
			$mail->send();
		...
		_::cache()->delete('ca1','friend_home_'.$f['ty'],0);
...
function setrec($id)
{
	...
	if($f=$db->findone('msn',array('_id'=>intval($id),'dd'=>array('$exists'=>false))))
	{
		if($f['fd']&&$f['pt'])
		{
			...
			if($mid=$db->insert('msn_rec',array('em'=>mb_strtolower($f['em'],'utf-8'),'ty'=>$f['ty'],'pr'=>$f['pr'],'ag'=>$f['ag'],'msn'=>intval($id))))
			{
				$photo=_::photo();
				$_fd = _::folder()->fd($mid);
