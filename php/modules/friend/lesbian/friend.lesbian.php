...
_::move('http://lesbian.boxza.com/friend',true);

define('F_TYPE','lesbian');

_::ajax()->register(array('sendreport','setrec'),'home');


_::$meta['title']='หาเพื่อนเลสเบี้ยน หาเพื่อนทอม หาเพื่อนดี้ - '._::$meta['title'];
...
if(!_::$content=$cache->get('ca1','friend_home_'.F_TYPE))
{
	...
	$template->assign('rec',$db->find('msn_rec',array('dd'=>array('$exists'=>false),'ty'=>F_TYPE,'fd'=>array('$exists'=>true)),array(),array('sort'=>array('_id'=>-1),'limit'=>30),false));
