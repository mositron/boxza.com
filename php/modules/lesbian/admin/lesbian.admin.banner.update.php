...
$db=_::db();
if(!$banner=$db->findone('lesbian_banner',array('_id'=>intval(_::$path[1]))))
{
	_::move('/');
}
...
if(isset($_FILES) && isset($_FILES['thumb']))
{
	...
	{
		$q=_::upload()->send('s3','upload','@'.$m,array('name'=>_::$path[1],'folder'=>'lesbian/banner','width'=>650,'height'=>385,'fix'=>'both','type'=>'jpg'));
		//if($p=$photo->thumb(_::$path[1],$m,'boyz/banner',650,385,'both','jpg'))
		if($q['status']=='OK')
		{
			$s['status']='OK';
			$s['pic']='http://s3.boxza.com/lesbian/banner/'.$q['data']['n'].'?'.rand(1,999);
		}
	}
	echo json_encode($s);
	exit;
...
//if($_POST)require_once(__DIR__.'/lesbian.admin.news.update.post.php');
