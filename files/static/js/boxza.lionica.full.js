_.lionica={
	...
	shop:
	{
		...
		buy:function(a,b,c,d,e)
		{
			...
			if(c>1)
			{
				c=Math.floor(prompt('ชื่อ '+n+' ในราคา '+d+' บ๊อก'+"\nใส่จำนวนที่ต้องการซื้อ",'1'));
			};
			if(c>0)
			{
				if(confirm('คุณต้องการซื้อ '+n+' '+c+' ชิ้น ในราคา '+(c*d)+' บ๊อก หรือไม่'))
				{
		...
		insert:function(e,i,c,p1)
		{
			...
			if(c&&c>0)
			{
				var p=Math.floor(prompt('ต้องการขาย '+$(e).find('span').html()+' ในราคาชิ้นละเท่าไหร่ (Box).',p1));
				if(p&&p>0)
				{
	...
	{	
		...
		obj.html=$('<i class="_plot _plot-m _plot-'+(obj.swing?'s':'m')+'-'+obj.face+(obj.attack?' lionica-popup _plot-obj':'')+'" style="left:'+(x*32)+'px;top:'+((y*32)-16)+'px; background-image:url(http://s0.boxza.com/static/images/game/lionica/monster/'+l3[1]+'.png);" data-popup="'+mo.name+' (Lv.'+mo.lv+')"></i>');
		$('.map').append(obj.html);
		...
		function start()
		{
			...
			if(obj.move)
			{
				...
				var b=_.lionica.map.loc[obj.to.x][obj.to.y];
				if((!b)||(b.substr(0,1)!='b'))
				{
			...
			{
				obj.face=move[Math.floor(Math.random()*move.length)];
				obj.html.prop('class','_plot _plot-m _plot-'+(obj.swing?'s':'m')+'-'+obj.face+(obj.attack?' lionica-popup _plot-obj':''));
			}
			if(v)
			{
		...
		function walking()
		{
			...
			if(sind<step)
			{
				...
				m.css({'left':pstep[0],'top':pstep[1]-16});
				tmr=setTimeout(function(){walking()},40);
			}
			else
			{
	...
	people:{
		...
		objpet:function(j,c)
		{
			...
			{
				cm.append('<i id="_player-'+j+'" data-popup="'+c.n+'" class="_player-obj" data-pet="'+j+'" data-x="'+c.x+'" data-y="'+c.y+'"><div><div id="_player-'+j+'-pet"></div><div id="_player-'+j+'-head" class="head"></div><div id="_player-'+j+'-guild" class="gname">'+(c.g?c.g:'')+'</div></div></i>');
			}
	...
	attacking:{
		...
		start:function(v)
		{
			//_.lionica.attacking.monster=v.monster;
		...
		stop:function(v)
		{
			...
			{
				mt.obj.prop('class','lionica-popup _plot _plot-m _plot-'+(mt.swing?'s':'m')+'-'+mf).css({'display':'','left':_.lionica.attacking.pos.monster[0],'top':_.lionica.attacking.pos.monster[1]});
			}
		...
		delay:function()
		{
			...
			if(_.lionica.attacking.index>=_.lionica.attacking.score.length)
			{
				...
				if(mt=_.lionica.attacking.monster)
				{
					mt.obj.prop('class','lionica-popup _plot _plot-m _plot-'+(mt.obj.data('swing')?'s':'m')+'-'+mt.obj.data('move')).css({'display':'none','left':mt.obj.data('posx')*32,'top':(mt.obj.data('posy')*32)-16});
					setTimeout(function(){mt.obj.css('display','')},3000);
				}
	...
	center:function(pos)
	{
		...
		$('#player').css({'left':pos[0]*32,'top':(pos[1]*32)-16});
	...
	hidesofar:function()
	{
		...
		for(i in _.lionica.monster)
		{
			if(_.lionica.monster.hasOwnProperty(i))
			{
				...
				x=Math.floor(Math.sqrt(((c.cur.x-p[0])*(c.cur.x-p[0]))+((c.cur.y-p[1])*(c.cur.y-p[1]))));
				if(x>12)
				{
					c.html.addClass('hideobj');
				}
				else
				{
					c.html.removeClass('hideobj');	
				}
			}
		}
	...
	findmon:function(cur)
	{
				...
				if(c.attack&&c.html.css('display')!='none')
				{
					...
					else if(x=_.lionica.routing(_.lionica.map.loc,p,c.cur))
					{
						if(x.length>1)
						{
							...
							else if(x.length==f && _.lionica.attacking.mons[c.cur.x+'_'+c.cur.y])
							{
								f=x.length;
								to=c;
							}
		...
		if(f>-1)
		{
			...
			_.lionica.logs([{'type':'','text':'<p class="logs-info">[AI] พบ '+to.name+' ['+(to.cur.x+1)+','+(to.cur.y+1)+']</p>'}]);
			...
			_.lionica.rindex=0;
			_.lionica.path=_.lionica.routing(_.lionica.map.loc,_.lionica.cur,[to.x,to.y]);
			_.lionica.walk();
		}
		else
		{
	...
	buy:function(a,b,c,d){var e=1;if(d<=0){e=Math.floor(prompt('ชื่อ '+$(a).text()+' ในราคา '+c+' บ๊อก'+"\nใส่จำนวนที่ต้องการซื้อ",'1'));};if(e>0){if(confirm('คุณต้องการซื้อ '+$(a).text()+' '+e+' ชิ้น ในราคา '+(c*e)+' บ๊อก หรือไม่')){_.lionica.api('buyitem',{'item':b,'count':e});}}},
	sell:function(a,b,c,d){if(d>1){d=Math.floor(prompt('ขาย '+$(a).text()+' ในราคา '+c+' บ๊อก'+"\nใส่จำนวนที่ต้องการขาย",d));};if(d>0){if(confirm('คุณต้องการขาย '+$(a).text()+' '+d+' ชิ้น ในราคา '+(c*d)+' บ๊อก หรือไม่')){_.lionica.api('sellitem',{'item':b,'count':d});}}},
	use:function(a,b){_.lionica.api('useitem',{'item':b});},
	skill:function(a,b){_.lionica.api('skill',{'type':a,'skill':a});},
	enhance:function(a,b,c,d){var n=$(a).find('span').text();if(confirm('ต้องการเพิ่มประสิทธิภาพให้กับ '+n+' จาก '+c+'->'+(c+1)+' โดยใช้ '+(d>1?'Defence':'Attack')+' Crystal กับเงิน 100 บ๊อก หรือไม่')){_.lionica.api('enhance',{'type':'inc','item':b});}},
	create:function(a)
	{
		$('#lionica').on({
			mouseenter: function()
			{
				...
				{
					$(this).prepend('<div id="lionica-balloon"></div>');
					$('#lionica-balloon').html($(this).data('popup'));
					...
					$('#lionica-balloon').css({'left':l,'top':t2});
				}
			},
			mouseleave: function()
			{
		...
		$('.lbox.lresize').resizable({
			stop:function(event,ui)
			{
				...
				{
					$(ui.element).find('.bresize').css({'height':h-30});
				}
			}
		});
			...
			for (x=0;x<h;x++)
			{
				...
				for(z=0;z<l2.length;z++)
				{
					...
					else if(l3[0]=='n')
					{
						...
						p +='<i class="lionica-popup _plot _plot-n" style="left:'+(x*32)+'px;top:'+((y*32)-16)+'px; background-position:'+mo.css+'" data-popup="'+mo.name+'"></i>';
					}
					else if(l3[0]=='m')
					{
	...
	charup:function()
	{
		var a=_.lionica.pet;
		$('#char_lv').html(a.lv);
		$('#char_hp').html(a.hp+' / '+a.mhp+' ('+Math.floor((a.hp/a.mhp)*100)+'%)').css({'background-position':Math.floor((a.hp/a.mhp)*215)});
		$('#char_mp').html(a.mp+' / '+a.mmp+' ('+Math.floor((a.mp/a.mmp)*100)+'%)').css({'background-position':Math.floor((a.mp/a.mmp)*215)});
		$('#char_exp').html(a.xp+' / '+a.mxp+' ('+Math.floor((a.xp/a.mxp)*100)+'%)').css({'background-position':Math.floor((a.xp/a.mxp)*215)});
