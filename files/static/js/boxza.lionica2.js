_.lionica={
	...
	render:{
		...
		start:function()
		{
			...
			if(dt>=100)
			{
				...
				_.lionica.render.renderer.render( _.lionica.render.scene, _.lionica.render.camera );
				_.lionica.stats.end();
			}
			//this.x += 10 * dt; 
		},
	...
	player:
	{
		...
		select:function()
		{
			...
			else
			{
				...
				_.lionica.eai=false;
				_.ajax.gourl('/lionica2/play','select');
			}
		},
		play:function()
		{
			...
			_.ajax.gourl('/lionica2/play','play',{'_id':_.lionica.player._id});
		},
		delete:function()
		{
			if(confirm('ต้องการลบตัวละครนี้หรือไม่'))
			{
				_.ajax.gourl('/lionica2/play','delete',{'_id':_.lionica.player._id});
			}
		},
		create:function()
		{
	...
	center:function()
	{
		...
		$('.minimap .name .text').html(_.lionica.map.name+': '+(p.cur[0]+1)+','+(p.cur[1]+1));
	...
	api:function(a,b)
	{
		if(_.lionica.loaded&&_.lionica.khash)
		{
			...
			_.ajax.gourl('/lionica2/play','socket',b);
			_.lionica.delay();
		}
	},
	...
	create:function(a)
	{
		...
		$(_.lionica.stats.domElement).css({'position':'absolute','right':5,'bottom':205}).appendTo($('#lionica'));
		...
		/*
		_.lionica.render.scene = new THREE.Scene();
	// CAMERA
	var SCREEN_WIDTH = $('#lionica').width(), SCREEN_HEIGHT = $('#lionica').height();
	var VIEW_ANGLE = 75, ASPECT = SCREEN_WIDTH / SCREEN_HEIGHT, NEAR = 0.1, FAR = 1000;
	_.lionica.render.camera = new THREE.PerspectiveCamera( VIEW_ANGLE, ASPECT, NEAR, FAR);
	//_.lionica.render.camera. = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.1, 1000);
	//_.lionica.render.scene.add(_.lionica.render.camera);
//	_.lionica.render.camera.position.set(0,150,400);
//	_.lionica.render.camera.lookAt(_.lionica.render.scene.position);	
	// RENDERER
	
	if (window.WebGLRenderingContext)
		_.lionica.render.renderer = new THREE.WebGLRenderer( {antialias:true} );
	else
		_.lionica.render.renderer = new THREE.CanvasRenderer(); 
	_.lionica.render.renderer.setSize(SCREEN_WIDTH, SCREEN_HEIGHT);
	$(_.lionica.render.renderer.domElement).css({'position':'absolute','left':0,'top':0,'z-index':999}).appendTo($('#lionica'));
	// EVENTS
	//THREEx.WindowResize(_.lionica.render.renderer, _.lionica.render.camera);
	//THREEx.FullScreen.bindKey({ charCode : 'm'.charCodeAt(0) });
	// CONTROLS
	var controls = new THREE.OrbitControls( _.lionica.render.camera, _.lionica.render.renderer.domElement );
	...
	var floorTexture = new THREE.Texture(canvas);
	...
	_.lionica.render.scene.add(mesh);
	
	
	var geometry = new THREE.CubeGeometry(1,1,1);
			var material = new THREE.MeshBasicMaterial({color: 0x00ff00});
			var cube = new THREE.Mesh(geometry, material);
			_.lionica.render.scene.add(cube);
	...
	*/
	...
	_.lionica.render.scene.add(_.lionica.render.camera);
	_.lionica.render.camera.position.set(0,-300,300);
	_.lionica.render.camera.lookAt(_.lionica.render.scene.position);	
...
_.lionica.render.scene.add(mesh);
		...
		//	_.lionica.render.camera.position.z = 5;

			var render = function () {
				requestAnimationFrame(render);

				controls.update();
				_.lionica.render.camera.up = new THREE.Vector3(0,1,0);
				document.title=_.lionica.render.camera.position.x+' - '+_.lionica.render.camera.position.y+' - '+_.lionica.render.camera.position.z;
			//	_.lionica.render.renderer.render(_.lionica.render.scene, _.lionica.render.camera);
			};
